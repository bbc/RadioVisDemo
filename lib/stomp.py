...
"""Stomp Protocol Connectivity
    ...
    * 2009/01/26 : (ChrisN) made the following changes:
       - removed try/except block from __receiver_loop to be able to see any errors that occur.
       - call __attempt_connection from the worker thread, to avoid blocking the thread that calls start().
       - allow connections to be routed through a proxy server, using SocksiPy module.
       - added attach_logger and detach_logger functions.
       - changed stop() method to only attempt disconnection if currently connected.
       - added enable_reconnect parameter to Connection __init__ method to allow prevention of reconnection attempts.
       - if available, use hashlib module instead of md5.
       - eliminated some pylint warnings.
"""

import math

# The md5 module is deprecated since version 2.5. Use the hashlib module instead.
try:
...
class Connection(object):
    ...
    def __init__(self, 
        ...
        \param enable_reconnect

                 if True, failed connection attempts are retried,
                 otherwise connections are only attempted once.
        ...
        self.__enable_reconnect = enable_reconnect
    ...
    def ack(self, headers=None, **keyword_headers):
        self.__send_frame_helper('ACK', '', self.__merge_headers([headers, keyword_headers]), [ 'message-id' ])
        
    def begin(self, headers=None, **keyword_headers):
    ...
    def abort(self, headers=None, **keyword_headers):
        self.__send_frame_helper('ABORT', '', self.__merge_headers([headers, keyword_headers]), [ 'transaction' ])
        
    def commit(self, headers=None, **keyword_headers):
            ...
            for host_and_port in self.__host_and_ports:
                try:
                    ...
                    if self.__proxy_settings is not None:
                        ...
                        self.__socket.setproxy(self.__proxy_settings['type'],
                                               self.__proxy_settings['host'],
                                               self.__proxy_settings['port'])
                    else:
...
if __name__ == '__main__':
    ...
    class StompTester(object):
        def __init__(self, host='localhost', port=61613, proxy_settings = None, user='', passcode=''):
            self.c = Connection(host_and_ports = [(host, port)],
    ...
    if "http" in proxies:
        ...
        print "Using http proxy: " + http_proxy.hostname + ", port " + str(http_proxy.port)
    ...
    st = StompTester(host, port, proxy_settings, user, passcode)
